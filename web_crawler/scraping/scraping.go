...
func new(searchLength int, pageLang string) *webPages {
	...
	return &webPages{
		...
		pageNumber:   0,
		pageLang:     pageLang,
		clearContent: make([][]string, searchLength),
	}
}

// Crawl performs url and text content search
func Crawl(pageLang string, searchLength int, startPage string, path string) {
	...
	for i := 0; i < wb.searchLength; i++ {
		...
		//	fmt.Println(wb.clearContent[i][j])
		//}
	}
	saveToFile(&*wb, path)
}
